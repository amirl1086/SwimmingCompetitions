        ...
        if(result != null) {
            try {
                ...
                while (competitionIds.hasNext()) {
                    ...
                    Competition comp = new Competition(currentCompetition);
                    if(this.currentUser.getType().equals("student")) {
                        if(comp.isCurrentUserRegistered(this.currentUser, comp.getParticipants())) {
                ...
                if(this.competitions.size() == 0) {
                    showToast("אין לך תוצאות שמורות של תחרויות במערכת");
                }
            }
            catch(Exception e) {
